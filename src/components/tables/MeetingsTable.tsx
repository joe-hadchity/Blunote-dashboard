...
export interface Meeting {
  id: string;
  recordingId?: string; // ID from recordings table (for routing to recording detail)
  title: string;
  description?: string;
  meetingLink?: string; // Google Meet/Zoom/Teams link for joining (from calendar sync)
  startTime: string;
  endTime: string;
  duration: number;
  type: "VIDEO" | "AUDIO";
  ...
  participants: string[];
  topics: string[];
  recordingUrl?: string; // Recording video/audio URL
  audioUrl?: string; // Separate audio URL
  syncedFromGoogle?: boolean;
  googleEventId?: string;
  isStandalone?: boolean; // True if recording has no meeting
  createdAt: string;
  updatedAt: string;
}
...
interface MeetingsTableProps {
  ...
  detailRoute?: string; // Custom route for detail pages (e.g., '/recording' or '/meeting')
}
...
export default function MeetingsTable({ 
  ...
  const fetchMeetings = useCallback(async (page = 1) => {
    ...
    try {
      ...
      // API now automatically filters for recordings only (via meetings_with_recordings view)
      // No need to pass hasRecording parameter anymore
      ...
      if (response.ok) {
        ...
        // Support both 'meetings' and 'recordings' response format
        setMeetings(data.meetings || data.recordings || []);
        if (data.pagination) {
  ...
  }, [sort, searchQuery, filters, apiEndpoint]);
  ...
  const handleRowClick = (meeting: Meeting) => {
    // ID is already set correctly in the API response
    // For recordings: id = recording_id
    // For meetings: id = meeting_id
    console.log('Navigating to recording:', meeting.id);
    router.push(`${detailRoute}/${meeting.id}`);
  };
  ...
  const handleDeleteMeeting = async (id: string) => {
    ...
    try {
      ...
      // Use the correct endpoint based on apiEndpoint prop
      const deleteEndpoint = apiEndpoint.replace('/recordings', `/recordings/${id}`).replace('/meetings', `/meetings/${id}`);
  ...
  const handleToggleFavorite = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      // Determine the correct endpoint based on apiEndpoint
      const favoriteEndpoint = apiEndpoint.includes('/recordings') 
        ? `/api/recordings/${id}/favorite`
        : `/api/meetings/${id}/favorite`;
      ...
      if (response.ok) {
        const data = await response.json();
        // Update meeting in current list - handle both response formats
        const newFavoriteStatus = data.meeting?.isFavorite ?? data.isFavorite;
        setMeetings(prev => prev.map(m => m.id === id ? { ...m, isFavorite: newFavoriteStatus } : m));
      } else {
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        console.warn('Failed to toggle favorite:', response.status, errorData);
        // Silently fail - don't throw error to avoid disrupting UI
      }
    } catch (error) {
      ...
      <div className="mb-6 flex flex-col sm:flex-row w-full items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Popover className="relative inline-block text-left">
            {({ close }) => (
              <>
                <Popover.Button className="inline-flex items-center gap-2 rounded-lg border border-gray-300 py-2 px-3 text-sm text-gray-700 hover:bg-gray-50 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600">
                  Filters
                  {activeFilterCount > 0 && (
                              ...
                              <Menu.Item>
                                {({ active }) => (
                                  <button 
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      console.log('Delete button clicked for ID:', meeting.id);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-gray-700">
            <div className="p-6">
              ...
              <p className="text-sm text-center text-gray-600 dark:text-gray-400 mb-6">
                This action cannot be undone. The recording file, transcripts, and any associated data will be permanently deleted.
              </p>
